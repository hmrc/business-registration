...
class ContactDetailsRepositorySpec extends UnitSpec with MongoSpecSupport with BeforeAndAfterAll with ScalaFutures with Eventually with WithFakeApplication {
  ...
  val ContactDetailsdata = ContactDetails("name",Some("name"),"sName", Some("email"), Some("num"), Some(""))
  val contactDetailsUpdated = ContactDetails("name2",Some("name"),"sName", Some("email"), Some("num"), Some("foo"))
  class Setup {
...
"upsertContactDetails" should {
  ...
  "update contact details succesfully not replacing valid data with blanks" in new Setup {
    await(repository.upsertContactDetails("regID", "intID", ContactDetailsdata))
    await(repository.count) shouldBe 1
    val res =  await(repository.upsertContactDetails("regID", "intID", ContactDetails("name1",None,"name2",None,None,None)))
    res shouldBe Some(ContactDetails("name1",Some("name"),"name2",Some("email"),Some("num"),Some("")))

  }

  "update Contact details should replace blank data with valid data" in new Setup {
    await(repository.upsertContactDetails("regID", "intID", ContactDetails("name1",None,"sName",None,None,None)))
  ...
  "should Successfully insert minimal required contact details" in new Setup {
    val res = await(repository.upsertContactDetails("regID", "intID", ContactDetails("name",None,"sName",None,None,None)))
    await(repository.count) shouldBe 1
    res shouldBe(Some(ContactDetails("name",None,"sName",None,None,None)))
  ...
  "getContactDetailsUnVerifiedUser" should {
    ...
    "return ContactDetails when internal id matches the one stored in mongo" in new Setup {
      await(repository.upsertContactDetails("reg1","int1",ContactDetails("foo",Some("mName"),"sNmee",Some("email"),Some("num1"),Some("num2"))))
      val c = await(repository.count)
      c shouldBe 1
      val res = await(repository.getContactDetailsUnVerifiedUser("reg1","int1"))
      res shouldBe Some(ContactDetails("foo",Some("mName"),"sNmee",Some("email"),Some("num1"),Some("num2")))
    }
  ...
  "getContactDetails" should {
    ...
    "return permission denied if user has incorrect internal id " in new Setup {
      await(repository.upsertContactDetails("foo","bar",ContactDetails("foo",None,"",Some("email"),Some("num1"),Some("num2"))))
